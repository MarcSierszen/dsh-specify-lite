...
export function resolveFeature(projectRoot, selector) { const data=features(projectRoot); if (typeof selector !== 'string' || !selector.trim()) throw new WorkflowError('INVALID_ARGUMENT','--selector is required'); if (isAbsolute(selector) || selector.split(/[\\/]/).includes('..')) throw new WorkflowError('UNSAFE_PATH','Feature selector must be project-relative and confined'); let candidates=[]; if (selector.startsWith('specs/')) { const expected=resolve(data.projectRoot,selector); if (!confined(data.specsRoot,expected)) throw new WorkflowError('UNSAFE_PATH','Feature selector escapes specs/'); if(hasSymlinkComponent(data.projectRoot,expected)) throw new WorkflowError('UNSAFE_PATH','Feature selector contains a symlink'); candidates=data.features.filter(x=>x.path===expected); } else candidates=data.features.filter(x=>x.name===selector || x.slug===selector); if (!candidates.length) throw new WorkflowError('NOT_FOUND','No matching feature directory'); if (candidates.length>1) throw new WorkflowError('AMBIGUOUS','Feature slug matches multiple features',{matches:candidates.map(x=>x.name)}); return {projectRoot:data.projectRoot,specsRoot:data.specsRoot,selector,feature:candidates[0]}; }
function parseTasks(text) { const lines=text.split(/\r?\n/), taskStart=lines.indexOf('## Task List'), verify=lines.indexOf('## Verification'); if (taskStart<0||verify<0||taskStart>=verify||lines.filter(x=>x==='## Task List').length!==1||lines.filter(x=>x==='## Verification').length!==1) throw new WorkflowError('MALFORMED_ARTIFACT','tasks.md must contain one ordered Task List and Verification section'); const tasks=[], ids=new Set(); for(let i=taskStart+1;i<verify;i++){const l=lines[i];if(!l.startsWith('- ['))continue;const m=TASK_RE.exec(l);if(!m)throw new WorkflowError('MALFORMED_ARTIFACT',`Malformed task at line ${i+1}`);const [,checked,id,refsRaw,textBody,depsRaw]=m;if(ids.has(id))throw new WorkflowError('MALFORMED_ARTIFACT',`Duplicate task ID ${id}`);ids.add(id);const references=[...refsRaw.matchAll(/(?:FR|SC|EC)-\d{3,}/g)].map(x=>x[0]); if(!refsRaw.includes('FOUNDATION')&&!references.length)throw new WorkflowError('MALFORMED_ARTIFACT',`Task ${id} lacks requirement mapping`); const dependencies=depsRaw?depsRaw.split(', '):[]; if(new Set(dependencies).size!==dependencies.length)throw new WorkflowError('MALFORMED_ARTIFACT',`Duplicate dependency on ${id}`); tasks.push({id,checked:checked==='x',references,dependencies,line:i+1,text:textBody});} if(!tasks.length)throw new WorkflowError('MALFORMED_ARTIFACT','tasks.md requires at least one task'); const pos=new Map(tasks.map((x,i)=>[x.id,i])); for(const [i,t] of tasks.entries())for(const d of t.dependencies){if(!pos.has(d))throw new WorkflowError('MALFORMED_ARTIFACT',`Unknown dependency ${d}`);if(d===t.id||pos.get(d)>=i)throw new WorkflowError('MALFORMED_ARTIFACT',`Dependency ${d} must precede ${t.id}`);} return {tasks,verification:lines.slice(verify+1).join('\n')}; }
...
export function selectTasks(projectRoot,tasksFile,selection=null){
  ...
  const {tasks}=parseTasks(readFileSync(real,'utf8'));
  const by=new Map(tasks.map(task=>[task.id,task]));
  let wanted;
  if(selection==null||selection==='') wanted=tasks.filter(task=>!task.checked).map(task=>task.id);
  else {
    wanted=[];
  ...
  return {projectRoot:root,tasksFile:real,requested:selection||null,selected,incompleteDependencies};
}
