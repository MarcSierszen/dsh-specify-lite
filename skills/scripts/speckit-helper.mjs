...
function parseArguments(argv) {
  ...
  if (!operation || !Object.hasOwn(OPERATION_FLAGS, operation)) {
    throw new WorkflowError('INVALID_ARGUMENT', operation ? `Unknown operation: ${operation}` : 'Operation is required');
  }
  ...
  for (let index = 0; index < tokens.length; index += 2) {
    ...
    if (!allowed.has(flag)) throw new WorkflowError('INVALID_ARGUMENT', `Unknown argument: ${flag}`);
    if (Object.hasOwn(values, flag)) throw new WorkflowError('INVALID_ARGUMENT', `Duplicate argument: ${flag}`);
    if (value === undefined || value.startsWith('--')) throw new WorkflowError('INVALID_ARGUMENT', `Missing value for ${flag}`);
    values[flag] = value;
  }

  for (const flag of ['--project-root', ...REQUIRED_FLAGS[operation]]) {
    if (!Object.hasOwn(values, flag) || values[flag] === '') throw new WorkflowError('INVALID_ARGUMENT', `${flag} is required`);
  }
...
function emitSuccess(operation, data, diagnostics = []) {
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, operation, data, diagnostics })}\n`);
}

function emitError(operation, error) {
  const code = error instanceof WorkflowError ? error.code : 'UNEXPECTED';
  const message = error instanceof Error ? error.message : String(error);
  const exitCode = ['INVALID_ARGUMENT', 'INVALID_SELECTION', 'NOT_FOUND', 'AMBIGUOUS'].includes(code)
    ? 2
    : code === 'UNSAFE_PATH'
      ? 3
      : ['PROJECT_CONFLICT', 'MALFORMED_FEATURE', 'MALFORMED_ARTIFACT'].includes(code)
        ? 4
        : 1;
...
try {
  ...
  if (operation === 'init-plan') {
    ...
    const selection = Object.hasOwn(parsed.values, '--selection') ? parsed.values['--selection'] : null;
